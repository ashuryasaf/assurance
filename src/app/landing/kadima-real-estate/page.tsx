...
const KEY_FACTS = [
  { icon: '✔', text: 'תוכנית מתאר מאושרת 2024' },
  { icon: '✔', text: 'החל מכ-320,000 ₪' },
  { icon: '✔', text: 'מחיר מוערך: כ-3,200 ₪ למ"ר' },
  { icon: '✔', text: 'פוטנציאל השבחה משמעותי' },
  { icon: '✔', text: 'ליווי משפטי ותכנוני מלא' },
];

const WHAT_YOU_GET = [
  { icon: '📄', title: 'שמאות', body: 'הערכת שווי מקצועית של הקרקע ע"י שמאי מקרקעין מוסמך.' },
  { icon: '📋', title: 'נסח טאבו', body: 'מידע רשמי על זכויות הבעלות, שעבודים והערות אזהרה.' },
  { icon: '🗺️', title: 'מידע תכנוני', body: 'תוכניות מתאר, ייעודי קרקע, זכויות בנייה וצפי פיתוח.' },
  { icon: '🤝', title: 'פגישת ייעוץ', body: 'פגישה אישית ללא התחייבות עם מומחה נדל"ן מוסמך.' },
];

const TESTIMONIALS = [
  { quote: 'קנינו חלקה במתחם H לפני שנה. כבר היום הערך עלה ב-25%. הליווי היה מושלם.', author: 'דני ורונית, כפר סבא' },
  { quote: 'הציגו לנו את כל המידע התכנוני בשקיפות מלאה. ידענו בדיוק מה אנחנו קונים.', author: 'מוטי, הרצליה' },
  { quote: 'ליווי משפטי מהשורה הראשונה. הרגשנו בטוחים לאורך כל הדרך.', author: 'שירה ואלון, רעננה' },
];

export default function KadimaRealEstateLandingPage() {
    ...
    try {
      ...
      const res = await fetch('/api/landing/real-estate', {
        ...
        body: JSON.stringify({
          ...
          campaignType: 'kadima-real-estate',
          source: 'kadima-real-estate-landing',
          referrer,
          utm,
          consent: true,
        }),
  ...
  const heroBackground = useMemo(
    () => ({
      backgroundImage:
        "radial-gradient(circle at 20% 80%, rgba(34,139,34,0.12) 0%, transparent 40%), radial-gradient(circle at 85% 0%, rgba(36,81,160,0.35) 0%, transparent 45%), linear-gradient(135deg, #0a1628 0%, #1e3a6e 55%, #0f2244 100%)",
    }),
    [],
  );
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f5f7fc', color: '#1e3a6e', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        ...
        <div style={{ maxWidth: '1180px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'minmax(0, 1.1fr) minmax(0, 1fr)', gap: '48px', alignItems: 'center' }}
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', color: '#dae8f8' }}>
              ...
              <span style={{ fontSize: '14px', fontWeight: 700, letterSpacing: '0.04em' }}>Assurance · קדימה נדל&quot;ן</span>
            </div>
            <h1 style={{ fontSize: '40px', fontWeight: 800, lineHeight: 1.15, marginBottom: '16px' }}>
              קרקע פרטית במתחם H <br />
              <span style={{ color: '#d4b44a' }}>קדימה צורן</span>
            </h1>
            <p style={{ fontSize: '17px', color: 'rgba(255,255,255,0.9)', maxWidth: '540px', lineHeight: 1.6, marginBottom: '24px' }}>
              אחד מאזורי הפיתוח המבוקשים ביותר בשרון.
              הזדמנות להיכנס מוקדם לאזור עם פוטנציאל עליית ערך יוצא דופן.
            </p>

            {/* Key facts */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '32px' }}>
              {KEY_FACTS.map(fact => (
                <div key={fact.text} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '16px' }}>
                  <span style={{ color: '#4CAF50', fontWeight: 800, fontSize: '18px' }}>{fact.icon}</span>
                  <span style={{ color: 'rgba(255,255,255,0.95)' }}>{fact.text}</span>
                </div>
              ))}
            </div>
      ...
      <section style={{ maxWidth: '1180px', margin: '-56px auto 0', padding: '0 24px' }}>
        <div style={{
          background: 'white', padding: '28px 24px', borderRadius: '20px',
          boxShadow: '0 24px 60px rgba(15,34,68,0.12)', position: 'relative', zIndex: 2,
        }}>
          <h2 style={{ fontSize: '22px', fontWeight: 800, color: '#1e3a6e', marginBottom: '20px', textAlign: 'center' }}>
            השאירו פרטים עכשיו לקבלת:
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }}>
            {WHAT_YOU_GET.map(item => (
              <div key={item.title} style={{ padding: '20px', background: '#f8fafe', borderRadius: '14px' }}>
      ...
      <section style={{ maxWidth: '1180px', margin: '64px auto 0', padding: '0 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
          {[
            ...
            ['2', '☎️ שיחת ייעוץ', 'נציג נדל"ן בכיר יחזור אליכם עם כל המידע על המתחם.'],
            ['3', '📋 קבלת חומרים', 'שמאות, נסח טאבו ומידע תכנוני — הכל שקוף ופתוח.'],
            ['4', '🤝 ליווי עד הרכישה', 'ליווי משפטי ותכנוני מלא עד סגירת העסקה.'],
          ].map(([n, title, body]) => (
            <div key={n} style={{
      ...
      <section style={{ maxWidth: '720px', margin: '64px auto', padding: '40px 24px', textAlign: 'center' }}>
        <div style={{
          ...
          <h3 style={{ fontSize: '22px', fontWeight: 800, marginBottom: '10px' }}>בונים את העתיד, חלקה אחר חלקה.</h3>
          <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.85)', marginBottom: '20px' }}>
            קדימה נדל&quot;ן — דקה למילוי, נציג חוזר אליכם בזמן הנוח.
          </p>
          <a href="#contact-form" style={{
            display: 'inline-block', padding: '14px 28px', borderRadius: '12px',
      ...
      <footer style={{ textAlign: 'center', padding: '28px 16px 48px', color: '#6b7a9a', fontSize: '12px' }}>
        © {new Date().getFullYear()} Assurance · קדימה נדל&quot;ן · אין מדובר בייעוץ או הצעת רכישה. הפנייה תיענה תוך 24 שעות בימי עסקים.
      </footer>
      ...
      <div id={id} style={cardStyle}>
        <div style={{ textAlign: 'center', padding: '20px 8px' }}>
          ...
          <p style={{ color: '#52668c', fontSize: '14px', lineHeight: 1.7 }}>
            נציג נדל&quot;ן בכיר יחזור אליך תוך 24 שעות עם שמאות, נסח טאבו ומידע תכנוני על מתחם H קדימה צורן.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <form id={id} onSubmit={submit} style={cardStyle}>
      <div style={{ marginBottom: '18px' }}>
        <h2 style={{ fontSize: '22px', fontWeight: 800, color: '#1e3a6e', marginBottom: '6px' }}>קבלו מידע על המתחם</h2>
        <p style={{ fontSize: '14px', color: '#52668c' }}>השאירו פרטים ונשלח שמאות, נסח טאבו ומידע תכנוני — ללא התחייבות.</p>
      </div>
      ...
      <FieldWrapper label="הערות (לא חובה)">
        <textarea
          value={form.notes}
          onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
          rows={3}
          placeholder="גודל חלקה מבוקש, תקציב, שאלות..."
      ...
      <button type="submit" disabled={submitting} style={{
        ...
        {submitting ? 'שולח...' : 'שלחו לי את החומרים'}
      </button>
