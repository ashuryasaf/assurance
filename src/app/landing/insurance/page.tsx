...
const HIGHLIGHTS = [
  { icon: '🛡️', title: 'כיסוי מקיף', body: 'ביטוח חיים, בריאות, סיעודי, רכב, דירה ועסק — הכל תחת קורת גג אחת עם התאמה אישית.' },
  { icon: '💰', title: 'חיסכון חכם', body: 'השוואת מחירים בין חברות הביטוח המובילות כדי למצוא לך את ההצעה המשתלמת ביותר.' },
  { icon: '🤝', title: 'ליווי אישי', body: 'סוכן ביטוח מוסמך ילווה אותך מהשלב הראשון ועד לטיפול בתביעות — בלי ביורוקרטיה.' },
  { icon: '📊', title: 'ניתוח תיק קיים', body: 'בדיקת כפלי ביטוח, פערי כיסוי והמלצות לאופטימיזציה של הפוליסות הקיימות שלך.' },
];

const TESTIMONIALS = [
  { quote: 'חסכנו 3,000 ₪ בשנה על ביטוח בריאות פשוט על ידי מעבר — בלי לוותר על כיסוי.', author: 'רחלי ויואב, פתח תקווה' },
  { quote: 'הסוכן טיפל לנו בתביעה תוך יומיים. שירות שלא מכירים בחברות הגדולות.', author: 'דוד, נתניה' },
  { quote: 'סוף סוף מישהו שהסביר לנו מה באמת צריך ומה מיותר. חוסכים ומרגישים מוגנים.', author: 'מיכל ואורן, ירושלים' },
];
    ...
    <main style={{ minHeight: '100vh', background: '#f5f7fc', color: '#1e3a6e', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        ...
        <div style={{ maxWidth: '1180px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'minmax(0, 1.1fr) minmax(0, 1fr)', gap: '48px', alignItems: 'center' }}
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', color: '#dae8f8' }}>
              ...
              <span style={{ fontSize: '14px', fontWeight: 700, letterSpacing: '0.04em' }}>Assurance · ביטוח</span>
            </div>
            <h1 style={{ fontSize: '44px', fontWeight: 800, lineHeight: 1.15, marginBottom: '20px' }}>
              ביטוח חכם ומותאם אישית, <br />
              <span style={{ color: '#d4b44a' }}>במחיר שמתאים לכם.</span>
            </h1>
            <p style={{ fontSize: '18px', color: 'rgba(255,255,255,0.85)', maxWidth: '540px', lineHeight: 1.6, marginBottom: '32px' }}>
              ביטוח חיים, בריאות, סיעודי, רכב, דירה ועסק — השוואה בין חברות הביטוח המובילות בישראל.
              השאירו פרטים ונציג מומחה יחזור אליכם בזמן שמתאים לכם.
            </p>

            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '24px', marginBottom: '36px' }}>
              {[
                ['🛡️', '+5,000 לקוחות', 'מבוטחים'],
                ['📞', '24 שעות', 'זמן מענה'],
                ['💰', 'עד 40%', 'חיסכון'],
              ].map(([icon, big, small]) => (
          ...
          <FormCard
            ...
            success={success}
            error={error}
            successMessage="תודה! קיבלנו את הפנייה. נציג ביטוח מומחה יחזור אליך תוך 24 שעות, בזמן שביקשת."
            ctaLabel="שלחו לי הצעת ביטוח"
          />
        </div>
      </section>
      ...
      <section style={{ maxWidth: '1180px', margin: '64px auto 0', padding: '0 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
          {[
            ...
            ['2', '☎️ שיחת ייעוץ', 'סוכן מוסמך יחזור אליכם לניתוח הצרכים.'],
            ['3', '📋 הצעות מותאמות', 'נשלח 2-3 הצעות ביטוח מהחברות המובילות.'],
            ['4', '✅ חתימה דיגיטלית', 'בחרתם? חותמים אונליין — בלי ביורוקרטיה.'],
          ].map(([n, title, body]) => (
            <div key={n} style={{
      ...
      <section style={{ maxWidth: '720px', margin: '64px auto', padding: '40px 24px', textAlign: 'center' }}>
        <div style={{
          ...
          <h3 style={{ fontSize: '22px', fontWeight: 800, marginBottom: '10px' }}>מוכנים לחסוך על הביטוח?</h3>
          <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.85)', marginBottom: '20px' }}>
            דקה למילוי, נציג מומחה חוזר אליכם בזמן הנוח.
          </p>
          <a href="#contact-form" style={{
            display: 'inline-block', padding: '14px 28px', borderRadius: '12px',
      ...
      <footer style={{ textAlign: 'center', padding: '28px 16px 48px', color: '#6b7a9a', fontSize: '12px' }}>
        © {new Date().getFullYear()} Assurance · אין מדובר בייעוץ ביטוחי. הפנייה תיענה תוך 24 שעות בימי עסקים.
      </footer>
    ...
    <form id={id} onSubmit={submit} style={cardStyle}>
      ...
      <FieldWrapper label="מה מעניין אתכם? (לא חובה)">
        <textarea
          ...
          rows={3}
          placeholder="סוג ביטוח, תקציב, כיסויים מבוקשים..."
