...
const TIME_LABELS: Record<RealEstateLead['preferredTime'], string> = {
  morning: 'בוקר 09:00 – 12:00',
  noon: 'צהריים 12:00 – 17:00',
  evening: 'ערב 17:00 – 20:00',
};

const STATUS_OPTIONS: { key: RealEstateLead['status']; label: string; color: string }[] = [
...
export default function LandingLeadsPage() {
  const [statusFilter, setStatusFilter] = useState<'all' | RealEstateLead['status']>('all');
  const [selected, setSelected] = useState<RealEstateLead | null>(null);
  const [convertId, setConvertId] = useState('');
  const [busy, setBusy] = useState(false);
  const [actionError, setActionError] = useState<string | null>(null);

  const { data, refresh, loading, error } = useApi<{ leads: RealEstateLead[] }>(
    `/api/landing/real-estate${statusFilter === 'all' ? '' : `?status=${statusFilter}`}`,
  );
  const leads = data?.leads ?? [];

  const updateStatus = async (lead: RealEstateLead, status: RealEstateLead['status']) => {
    setActionError(null);
    setBusy(true);
    try {
  ...
  const convertToCrm = async (lead: RealEstateLead) => {
    setActionError(null);
    if (!convertId.trim()) {
      ...
      <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <p style={{ color: '#6b7a9a', fontSize: '15px' }}>
            פניות שהגיעו דרך עמוד הנחיתה החיצוני (/landing/real-estate). ניתן להמיר ל־CRM לאחר קבלת ת.ז.
          </p>
        </div>
        <a href="/landing/real-estate" target="_blank" rel="noopener noreferrer" style={{
          padding: '10px 18px', borderRadius: '10px', background: 'linear-gradient(135deg, #c9a227, #a87c1a)',
          color: 'white', fontWeight: 700, textDecoration: 'none', alignSelf: 'flex-start',
        }}>
          🌐 צפה בדף הנחיתה ↗
        </a>
      </div>
      ...
      <div className="card" style={{ padding: '0', overflow: 'hidden' }}>
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['שם מלא', 'טלפון', 'אימייל', 'שעה מועדפת', 'סטטוס', 'מקור', 'התקבל'].map(h => (
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
              <div>
                ...
                <div style={{ fontSize: '13px', color: '#6b7a9a' }}>
                  התקבל: {new Date(selected.createdAt).toLocaleString('he-IL')}
                </div>
              </div>
...
function StatusBadge({ status }: { status: RealEstateLead['status'] }) {
