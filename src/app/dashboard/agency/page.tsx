...
export default function AgencyPage() {
  ...
  const { data, loading, error } = useApi<{ agencies: ApiAgency[] }>('/api/agencies');
      ...
      {showAddAgent && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '32px', maxWidth: '500px', width: '90%' }}
            ...
            <p style={{ color: '#6b7a9a', fontSize: '14px', marginBottom: '20px' }}>
              כדי להוסיף סוכן, הנפק קישור הזמנה תחת תפריט הסוכנות. המערכת תייצר קישור ייחודי לרישום עצמי.
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowAddAgent(false)} style={{ padding: '10px 20px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #1e3a6e, #2451a0)', color: 'white', fontWeight: '700', cursor: 'pointer' }}>{t('cancel')}</button>
            </div>
          </div>
        </div>
      )}
